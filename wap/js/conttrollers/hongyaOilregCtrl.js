...
define([
    ...
    controllers.controller('hongyaOilregCtrl', ['$scope', '$rootScope', 'resourceService', '$filter', '$state', 'ngDialog', '$location', 'md5', '$localStorage', '$stateParams','signWeChatService', function ($scope, $rootScope, resourceService, $filter, $state, ngDialog, $location, md5, $localStorage, $stateParams,signWeChatService) {
        // signWeChatService();
        if ($filter('isRegister')().register) {
            $scope.user = $filter('isRegister')().user.member;
        }
        $rootScope.title = '啄米理财-350元油卡等你来';
        ...
        $scope.zuce = function (tegForm) {
            ...
            $scope.login.toFrom = 'chedaojiayou';
            if (tegForm.$valid) {
        ...
        wx.ready(function () {
            wx.onMenuShareTimeline({
                title: '啄米理财，国企背景',
                desc: '350元油卡等你来抢。', // 分享描述
                link: 'http://wap.huiyuanjinfu.com/oilreg', // 分享链接
                imgUrl: 'http://wap.huiyuanjinfu.com/images/wechat-share/reg-oil.png', // 分享图标
                success: function () {
                    alert('分享成功！');
                },
                cancel: function () {
